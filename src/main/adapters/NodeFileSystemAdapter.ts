...
export class NodeFileSystemAdapter implements IFileSystemPort {
  ...
  async moveToTrash(path: string): Promise<void> {
    try {
      ...
      if (error.message && (error.message.includes('permisos') || error.message.includes('permission'))) {
        ...
        try {
          ...
          throw new Error('No se pudo mover la aplicación a la papelera. La aplicación puede estar en uso o protegida por el sistema.');
        }
      } else {
        throw error;
      }
  ...
  private async moveToTrashWithAdminPrivileges(path: string): Promise<void> {
    ...
    try {
      ...
      if (error.message && (error.message.includes('User canceled') || error.message.includes('cancelado'))) {
